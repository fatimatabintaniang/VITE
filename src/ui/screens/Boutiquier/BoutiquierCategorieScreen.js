...
export default class CategoryScreen {
  constructor(root) {
    this.root = root;
    this.catSvc = new CategoryService(); // ✅ Utilise l’URL par défaut : "http://localhost:3000/categories"
  ...
  try {
    ...
    this.state.total = +headers["x-total-count"] || this.state.list.length; // FIXED
    console.log("🔍 Données reçues de catSvc.list():", data);
console.log("🧾 Vue actuelle:", this.state.view);
console.log("📦 Liste filtrée:", this.state.list);
